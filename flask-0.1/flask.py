...
class _RequestContext(object):
    ...
    def __enter__(self):
        # _request_ctx_stack 是 class werkzeug.local.LocalStack 的实例, 在此文件的末尾处声明
...
def flash(message):
    ...
    """
    # flash 信息在 session 对象中存储为一个 list
...
# 获取 package 的绝对路径, 否则返回 os.getcwd()
...
class Flask(object):
    ...
    # 创建或打开一个 session
    # 此处的实现方式好像是把 session 存储到了 cookie 中
    def open_session(self, request):
        ...
        if key is not None:
            return SecureCookie.load_cookie(request, self.session_cookie_name,
                                            secret_key=key)
    # 保存 session, 将 session 内容写入 response
    ...
    def wsgi_app(self, environ, start_response):
        ...
        """
        with self.request_context(environ):     # 载入上下文
            rv = self.preprocess_request()      # 执行预处理函数
            if rv is None:
                rv = self.dispatch_request()    # 处理请求
            response = self.make_response(rv)   # 创建 response
            response = self.process_response(response)  # 执行后处理函数
            return response(environ, start_response)    # 返回 response

    # 根据环境信息创建请求上下文, 并绑定到当前上下文
