...
# 这里的 json 库优先选择使用 simplejson, 关于 simplejson 和标准库 json 两者的区别
# 在 Py2.7 上, simplejson 优于 json, 在 Py3.5 上, json 优于 simplejson
# 详细信息参考: https://artem.krylysov.com/blog/2015/09/29/benchmark-python-json-libraries/
json_available = True
try:
...
from werkzeug import abort, redirect    # 导入但并没有使用
from jinja2 import Markup, escape       # 导入但并没有使用
...
# 虽然 flash 添加了 category 参数, 但是该函数获取 flash 信息是默认不附带此信息
...
# 默认模板上下文处理器, 注入 request, session and g, 然后再模本定义中就可以使用这个内容了
...
class Flask(_PackageBoundObject):
    ...
    # 默认的 session 实现是把 session 存入 cookie 中
